...
const emit = defineEmits(["cambiarComponente", "editarUsuario"]);
const irARegistrarUsuario = () => {
  emit("cambiarComponente");
};
const { obtenerUsuarios } = useUsuarios();

const { data: usuarios, error: errorUsuarios } = await useAsyncData<Usuario[]>("usuarios", obtenerUsuarios);
if (errorUsuarios.value) {
  console.error("Error al obtener los usuarios:", errorUsuarios.value);
}
        ...
        <tbody>
          ...
          >
            ...
            <td class="py-2 px-2 flex justify-end space-x-2">
              ...
              <button class="text-primary hover:text-red-700 "  @click="$emit('editarUsuario', usuario)"
              >
